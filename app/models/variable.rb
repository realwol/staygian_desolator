class Variable < ActiveRecord::Base
  ...
  def self.create_product_variable(options, product)
    ...
    options.delete_if{|i| i.blank?}[1..-1].each do |variable|
      ...
      variable_hash[:image_url4] = variable[:image_url4]
      variable_hash[:image_url5] = variable[:image_url5]
      variable_hash[:image_url6] = variable[:image_url6]
  ...
  private
    def self.variable_params option
      option.permit( :title, :color, :size, :price, :product_id, :deleted_at, :stock, :update_status, :translate_status,
                                       :image_url1, :image_url2,:image_url3,:image_url4, :image_url5,:image_url6, :image_url7, :image_url8,:image_url9,
                                       # :image_url10,
