namespace :update_shop_charger do
  ...
  task run: :environment do
    Vendor.all.each do |vendor|
      ...
      vendor_product_count_array = Product.find_by_sql("select user_id, count(user_id) as user_count from products where shop_id = #{shop_id} group by user_id order by user_count ")
      if vendor_product_count_array.present?
        charger_info = vendor_product_count_array.first
